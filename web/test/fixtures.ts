...
import type {
  GridResponse, GuideResponse, LookupResult, LookupTask, OptionsResponse, Source, Vehicle,
} from "../src/api";
...
const task = (name: string, miles: number, extra: Partial<LookupTask> = {}): LookupTask => ({
  ...
  task_name: name, category: "Diagnostics & Inspection", priority: "Recommended", menu: "Normal",
...
export const gridResponse: GridResponse = {
  vehicle, source,
  resolution: { matched_configs: 1, relaxed_fields: [] },
  mileage: { current: 70000, current_interval: 70000 },
  columns: [
    { mileage: 65000, label: "65,000" },
    { mileage: 70000, label: "70,000", current: true },
    { mileage: 75000, label: "75,000" },
  ],
  rows: [
    {
      taskName: "Replace engine oil and oil filter", category: "Oil & Consumables",
      advisor_label: null, advisor_rank: 1,
      cells: { "65000": false, "70000": true, "75000": false },
      details: [{ mileage: 70000, task_key: "oil-70000", service_id: null, priority: "Recommended", menu: "Normal", menu_price_cents: null }],
    },
    {
      taskName: "Rotate tires", category: "Wheels & Tires",
      advisor_label: "TIRE ROT", advisor_rank: 2,
      cells: { "65000": true, "70000": true, "75000": true },
      details: [{ mileage: 70000, task_key: "rot-70000", service_id: null, priority: "Recommended", menu: "Normal", menu_price_cents: 2995 }],
    },
  ],
};
...
export const guideResponse: GuideResponse = {
  ...
  guide: {
    sections: [
      ...
      { id: "due_now", title: "Due now", paragraphs: ["At 70,000 miles, this schedule shows oil and filter service."],
        items: [{ label: "Rotate tires — TIRE ROT", tags: ["recommended"] }] },
...
export function installFetchMock(vi: typeof import("vitest").vi) {
  ...
  vi.stubGlobal("fetch", (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.startsWith("/api/models")) return respond({ year: 2020, models: ["4RUNNER", "CAMRY", "TUNDRA"] });
